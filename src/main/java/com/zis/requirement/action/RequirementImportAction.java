...
public class RequirementImportAction extends ActionSupport {
	...
	private Logger logger = Logger.getLogger(RequirementImportAction.class);
	...
	public String upload() {
		try {
			...
			List<RequirementImportDTO> list = im.parse(billInstance,
					propMapping);
	...
	private void saveImportRecord(List<RequirementImportDTO> list) {
		for (RequirementImportDTO record : list) {
			...
			List<Bookinfo> bookList = bookService.findBookByISBN(record
					.getIsbn());
	 ...
	 */
	private FileImporter<RequirementImportDTO> initExcelImporter()
			throws FileNotFoundException {
		FileImporter<RequirementImportDTO> im = new ExcelImporter<RequirementImportDTO>(
				new FileInputStream(excelFile), "bookRequirement.xls");
