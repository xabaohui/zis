...
public class BookInfoToolkit {
	...
	public List<String> updateFixBookName(String startLabel, String keyword) {
		if (StringUtils.isBlank(startLabel) || StringUtils.isBlank(keyword)) {
			String errMsg = String
					.format("illegal arguments, for input startLabel=%s, keyword=%s", startLabel, keyword);
	...
	public List<String> updateFixEditionByBookName(String keyword) {
		...
		for (Bookinfo book : list) {
			...
			this.bookService.updateBook(book);
			String infoMsg = String.format("update book %s,%s to %s,%s where id=%s", orig, origEdition, newName,
					newEdition, book.getId());
			logger.info(infoMsg);
			result.add(infoMsg);
		}
		return result;
	...
	public List<String> updateFixBookAuthor(List<Bookinfo> bookList) {
		...
		for (Bookinfo book : bookList) {
			...
			try {
				...
				this.bookService.updateBook(book);
				String infoMsg = String.format("update bookAuthor %s to %s where id=%s", origAuthor, destAuthor,
						book.getId());
				logger.info(infoMsg);
				result.add(infoMsg);
			} catch (Exception e) {
