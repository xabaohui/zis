	...
	public void abstractAddAllProcessingItems(final ShopInfo shop) {
		Thread task = new Thread(new Runnable() {
			public void run() {
				...
				do {
					pageList = shopItemMappingDao.findByShopIdAndSystemStatus(shop.getShopId(),
...
//	}
	...
	private void saveWaitShopItemMapping(ShopItemMapping mapping) {
	...
	// * @param shop
	// */
	// public void downloadItems2Mapping(ShopDownloadInterfaceDto dto, ShopInfo
	// shop) {
	...
	// }
	// Bookinfo book =
	// this.bookService.findByIdAndIsbn(Integer.parseInt(bookId), isbn);
	// // 对应商家编码在数据库中无数据，证明商家编码为乱填，加入错误待生成邮件内容中
	// if (book == null) {
	// String msg = String.format("%s  %s  %s <p/>\n", s.getTitle(),
	// s.getItemOutNum(), "商家编码有误");
	// sb.append(msg);
	// logger.error(msg);
	// continue;
	// }
