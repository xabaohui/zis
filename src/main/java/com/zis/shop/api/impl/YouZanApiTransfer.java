...
public class YouZanApiTransfer extends AbstractApiTransfer {
	...
	private static final Logger logger = LoggerFactory.getLogger(YouZanApiTransfer.class);
	...
	public Long addItem(ApiAddItemDto apiItemDto, ShopInfo shop) {
		...
		kdtItemAddParams.setIsDisplay(0L);//TODO 上线前修改为1L为默认上架
	 ...
	 * 上架商品
	 * 
	 * @param shopItemMapping
	 * @param shop
	 * @return
	 */
	private boolean upLoadItem(ShopItemMapping shopItemMapping, ShopInfo shop) {
		KDTClient client = new DefaultKDTClient(getSign(shop));
	 ...
	 * @return
	 */
	private ApiQueryItemsDto buildOnsaleItems(
			com.youzan.open.sdk.gen.v1_0_0.model.KdtItemsOnsaleGetResult.GoodsDetail[] result, Long totalResults,
			ShopInfo shop) {
	 ...
	 * @return
	 */
	private ApiQueryItemsDto buildInventoryItems(
			com.youzan.open.sdk.gen.v1_0_0.model.KdtItemsInventoryGetResult.GoodsDetail[] result, Long totalResults,
			ShopInfo shop) {
		ApiQueryItemsDto dto = new ApiQueryItemsDto();
		List<ShopItemMapping> list = new ArrayList<ShopItemMapping>();
		dto.setTotalResults(totalResults);
	 ...
	 * @return
	 */
	private Sign getSign(ShopInfo shop) {
