...
public class JiShiBaoApiTransfer extends AbstractApiTransfer {
	...
	private static Logger logger = LoggerFactory.getLogger(JiShiBaoApiTransfer.class);

	private final Long DEFAULT_PAGE_SIZE = 40L;

	private final String ZIS = "已同步到zis";
	...
	public List<CreateTradeOrderDTO> queryTradeForDate(ShopInfo shop, Date startTime, Date endTime) {
		return queryTradeForDate(null, null, shop, startTime, endTime);
	}

	public List<CreateTradeOrderDTO> queryTradeForDate(List<CreateTradeOrderDTO> oldlist, Long page, ShopInfo shop,
		...
		try {
			...
			String msg = String.format("%s %s %s %s %s %s", "获取订单失败 开始时间", dateToString(startTime), "结束时间",
					dateToString(endTime), "错误原因", e.getMessage());
			logger.error(msg, e);
			throw new RuntimeException(msg);
		}
	}

	private List<CreateTradeOrderDTO> buildCreateTradeOrderDTO(ShopInfo shop, List<CreateTradeOrderDTO> dtoList,
		...
		for (Trade t : tradeList) {
			...
			if ((t.getHasBuyerMessage() != null && t.getHasBuyerMessage())
					|| (t.getSellerRate() != null && t.getSellerRate())
					|| (t.getSellerCanRate() != null && t.getSellerCanRate())) {
	 ...
	 * @return
	 */
	private Trade getTradeGet(ShopInfo shop) {
		try {
			JSBClient client = getClient(shop);
	...
	public List<ApplyRefundDTO> queryApplyRefundForDate(ShopInfo shop, Date startTime, Date endTime) {
		return queryApplyRefundForDate(null, null, shop, startTime, endTime);
	}

	private List<ApplyRefundDTO> queryApplyRefundForDate(Long page, List<ApplyRefundDTO> oldlist, ShopInfo shop,
			Date startTime, Date endTime) {
		try {
			if (page == null) {
				page = 1L;
			...
			String msg = String.format("%s %s %s %s %s %s", "获取退款订单失败 开始时间", dateToString(startTime), "结束时间",
					dateToString(endTime), "错误原因", e.getMessage());
			logger.error(msg, e);
			throw new RuntimeException(msg);
		}
	...
	private List<ApplyRefundDTO> buildApplyRefundDTO(ShopInfo shop, List<ApplyRefundDTO> aRList, List<Refund> rlist) {
