...
public class StorageChangeTimer extends CommntOrderTimer {
	...
	private static Logger logger = Logger.getLogger(StorageChangeTimer.class);
	...
	public void stockChangeToCompany() {
		...
		for (Company c : comList) {
			try {
				...
				List<StorageProduct> pList = this.storageService.findByUpdateTimeBetweenStartTimeAndEndTimeAndRepoId(
						startTime, endTime, sList.get(0).getRepoId());
				if(pList.isEmpty()){
					continue;
				}
				for (StorageProduct s : pList) {
					this.doShopService.stockChangeToShopUPdateItem(c.getCompanyId(), s.getSkuId(),
							s.getStockAvailable());
				}
			} catch (Exception e) {
