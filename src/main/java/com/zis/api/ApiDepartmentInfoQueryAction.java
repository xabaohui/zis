...
public class ApiDepartmentInfoQueryAction extends BaseApiAction {
	...
	private static final Logger logger = Logger
			.getLogger(ApiDepartmentInfoQueryAction.class);
	...
	public String queryCollege() {
		...
		dc.setProjection(Projections.projectionList()
				.add(Projections.groupProperty("college"))
	...
	private List querySpecifiedInstitute() {
		...
		if (di == null) {
			// 如果查不到院校信息，则返回空list
			logger.warn("api invoke warn, no such departmentInfo where id = "
					+ departId);
	...
	// 查询指定学校、学院下的所有专业
	private List querySpecifiedDepartment() {
		Integer departId = Integer.parseInt(id);
		Departmentinfo di = this.schoolBiz.findDepartmentInfoById(departId);
		if (di == null) {
			// 如果查不到院校信息，则返回空list
