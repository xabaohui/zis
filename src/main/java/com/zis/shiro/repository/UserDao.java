...
public interface UserDao extends PagingAndSortingRepository<User, Integer> {
	...
	@Query(value = "SELECT new com.zis.shiro.dto.ShiroRealmDto(ut.id, ut.userName, ut.password, ut.salt, rt.roleName, pt.permissionName, pt.url) "
			...
			+ "WHERE rt.id= ut.roleId AND rt.id = rpt.roleId AND "
			+ "pt.id = rpt.permissionId AND ut.userName = :userName" + " AND ut.isDelete = '" + NO + "'")
	...
	@Query(value = "SELECT new com.zis.shiro.dto.UpdateUserInfo(ut, rt) " + "FROM User ut, Role rt "
			...
			+ " AND ut.isDelete = '" + NO + "' ORDER BY ut.updateTime DESC")
	 ...
	 */
	@Query(value = "SELECT new com.zis.shiro.dto.UpdateUserInfo(ut, rt) " + "FROM User ut, Role rt "
			+ "WHERE rt.id= ut.roleId AND ut.userName <> '" + ADMIN + "' AND ut.isDelete = '" + NO
