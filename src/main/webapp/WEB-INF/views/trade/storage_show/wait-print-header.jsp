		...
		<table id="common-table">
			...
			<c:forEach items="${orderList}" var="order">
				<tr>
					...
					<td><c:if test="${order.canPrint()}">
							<input type="button" value="打单" onclick="printExpress('${order.id}', 'getWaitForPrintList')" />
						&nbsp;
					</c:if>
						<div id="desc_${order.id}">
							<c:if test="${not empty order.salerRemark}">
								<span title="${order.salerRemark}" onclick="showAppendSellerRemarkView('${order.id}')"><font
									color="red">备注</font>
								</span>
							</c:if>
