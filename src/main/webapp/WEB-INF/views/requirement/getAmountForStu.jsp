...
<html>
	...
	<h1>图书使用量 - 学生版</h1>
		...
		<c:if test="${not empty prePage}">
			<a href="requirement/getAmountForStu?page=${prePage}&bookName=${bookName}&school=${school}&institute=${institute}&partName=${partName}&operator=${operator}">上一页</a>&nbsp;
		</c:if>
		${page}
		&nbsp;
		<c:if test="${not empty nextPage}">
			<a href="requirement/getAmountForStu?page=${nextPage}&bookName=${bookName}&school=${school}&institute=${institute}&partName=${partName}&operator=${operator}">下一页</a>&nbsp;
		</c:if>
		<!-- 分页查询end -->
		</div>
